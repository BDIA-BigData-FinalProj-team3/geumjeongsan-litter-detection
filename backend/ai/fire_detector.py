...
def analyze_fire_images(image_paths, wind_dir, wind_speed, humidity):
    ...
    # 2. 프롬프트 구성
    prompt = f"""🔥 산불 조기 감지 전문 프롬프트 '민감 모드' (v2.6 - 미탐 방지 강화 + 입김/안개 오탐지 방지)
...
[분석 지침]
**중요**: 짧은 간격(예: 2초)으로 촬영된 연속 프레임(보통 5장)이 제공됩니다.
단, 영상의 마지막 구간에서는 일부 프레임이 누락되어 2~4장만 제공될 수 있습니다. 제공된 프레임 범위 내에서 최선의 판단을 하십시오.
프레임 간 비교를 통해:
...
• 특정 지점(산 능선/숲/계곡 등)에서 **반복적으로 솟아오르는 연기**가 보이는 경우
• 바람 방향과 일치하는 방향으로 연기가 **서서히 확산**되는 경우

만약 화재/연기 여부가 애매하면, `is_fire_detected`를 무조건 false로 하지 말고
`needs_detailed_inspection=true`로 설정하여 추가 확인이 필요하다고 표시하십시오.
...
"""
    ...
    response_schema = {
        ...
        "properties": {
            ...
            "detections": {
                "type": "array",
                "description": "detected_frame_index 기준 fire/smoke bbox 목록(0~3개)",
                "items": {
                    "type": "object",
                    "properties": {
