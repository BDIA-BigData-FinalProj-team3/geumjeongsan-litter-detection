...
export default function FireDashboard({ onNavigate }: FireDashboardProps) {
  ...
  const [weather, setWeather] = useState<{ windSpeed?: number } | null>(null);

  // API에서 데이터 로드
  useEffect(() => {
    const loadFireData = async () => {
      const [active, completed, statsData, hotspotsData, weatherData] = await Promise.all([
        getActiveFires(),
        getCompletedFires(),
        getFireStats(),
      ...
      console.log('✅ [Fire] All data loaded:', { 
        stats: statsData, 
  ...
  // viewMode에 따라 필터링
  const firesByStatus = viewMode === 'active' ? activeFires : completedFires;
  
  // 통합 검색 필터링
  const filteredFires = searchQuery.trim()
    ? firesByStatus.filter(fire => {
        const query = searchQuery.toLowerCase();
        return (
          fire.accidentCode?.toUpperCase().includes(searchQuery.toUpperCase()) ||
          fire.cctvId?.toLowerCase().includes(query) ||
          fire.location?.toLowerCase().includes(query) ||
          fire.handler?.toLowerCase().includes(query) ||
          fire.status?.toLowerCase().includes(query) ||
          fire.severity?.toLowerCase().includes(query)
        );
      })
    : firesByStatus;
  ...
  const handleSearch = (query: string) => {
    ...
    // 검색 결과 확인
    const currentFires = viewMode === 'active' ? activeFires : completedFires;
    const lowerQuery = trimmedQuery.toLowerCase();
    const hasResults = currentFires.some(fire => 
      fire.accidentCode?.toUpperCase().includes(trimmedQuery.toUpperCase()) ||
      fire.cctvId?.toLowerCase().includes(lowerQuery) ||
      fire.location?.toLowerCase().includes(lowerQuery) ||
      fire.handler?.toLowerCase().includes(lowerQuery) ||
      fire.status?.toLowerCase().includes(lowerQuery) ||
      fire.severity?.toLowerCase().includes(lowerQuery)
    );

    if (!hasResults) {
      // 다른 모드에서 결과가 있는지 확인
      const otherFires = viewMode === 'active' ? completedFires : activeFires;
      const hasResultsInOther = otherFires.some(fire => 
        fire.accidentCode?.toUpperCase().includes(trimmedQuery.toUpperCase()) ||
        fire.cctvId?.toLowerCase().includes(lowerQuery) ||
        fire.location?.toLowerCase().includes(lowerQuery) ||
        fire.handler?.toLowerCase().includes(lowerQuery) ||
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-2">
              ...
              <div className="bg-white py-2 px-4 shadow-sm border border-gray-200 rounded-lg">
                ...
                <div className="text-gray-900 text-xl font-semibold">
                  {weather?.windSpeed ? `${(weather.windSpeed * 3.6).toFixed(1)}km/h` : '-'}
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedFires.length === 0 ? (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-gray-900">{fire.accidentCode}</td>
                        ...
                        <td className="px-3 py-2 text-gray-600 text-sm">{fire.time}</td>
                        {viewMode === 'completed' && (
                          <>
                            <td className="px-3 py-2 text-gray-600 text-sm">{fire.responseTime || '-'}</td>
                            <td className="px-3 py-2 text-gray-600">{fire.duration || '-'}</td>
                          </>
                        )}
                        <td className="px-3 py-2">
                        ...
                        <td className="px-3 py-2 text-gray-600">{fire.handler}</td>
                      </tr>
                      );
                      })
                    )}
